...
def load_files():
    ...
    # Load Encoders and Column Names
...
# Load all files
model_rf, encoders, columns = load_files()
...
# -------------------------------
# Input Form
# Using a form prevents rerunning after each selection.
# The app runs only when the Predict button is clicked.
# -------------------------------
with st.form("prediction_form"):
    ...
    with col2:
        ...
        additional_info = st.selectbox(
            "Additional Info",
            encoders["Additional_Info"].classes_
        )
...
# Show duration outside form
st.info(
    f"⏱️ Duration: {duration // 60}h {duration % 60}m ({duration} minutes)"
)
...
if submitted:
    ...
    st.markdown(
        ...
        ">
            ✨💖 Estimated Flight Price: Rs {round(prediction, 2):,.2f} 💖✨
        </div>
        """,
        unsafe_allow_html=True
    )
